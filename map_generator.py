...
def map_image_generator(sub_key, trj_id, latitude_origin, longitude_origin, latitude_dest, longitude_dest):
    ...
    response = requests.post('https://atlas.microsoft.com/mapData/upload?subscription-key='+sub_key+'&api-version=1.0&dataFormat=geojson', json=data, timeout=5.0)
    ...
    if not response.status_code == requests.codes.ok:
        ...
        response_2 = requests.get(resp_url+status_uri+'?api-version=1.0&subscription-key='+sub_key, timeout=1.0) 
        ...
        if not response_2.status_code == requests.codes.ok:
            ...
            map_response = requests.get('https://atlas.microsoft.com/map/static/png?subscription-key='+sub_key+'&api-version=1.0&'+style+'udid-'+udId, timeout=1.0)
